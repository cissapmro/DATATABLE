...
const Crud = () => {
    ...
    const [usuarioDialog, setUsuarioDialog] = useState(false); //mensagem
    const [deleteUsuarioDialog, setDeleteUsuarioDialog] = useState(false); //deletar um usuário
    const [deleteUsuariosDialog, setDeleteUsuariosDialog] = useState(false); //deletar vários usuários
    const [usuario, setUsuario] = useState<Projeto.Usuario>(usuarioVazio); //inserir um usuário
    const [selectedUsuarios, setSelectedUsuarios] = useState(null); //listar
    const [submitted, setSubmitted] = useState(false); //enviar
    const [globalFilter, setGlobalFilter] = useState(''); //filtrar
    ...
    useEffect(() => {
        const listarUsuarios = async () => {
            try {
                const dadosUsuarios = await usuarioService.getLogin(); // Chama o serviço para obter os usuários
                setUsuarios(dadosUsuarios); // Atualiza o estado de 'usuarios' com os dados recebidos
            } catch (error) {
                console.error("Erro ao obter informações de login:", error);
            }
        };
        listarUsuarios(); // Chama a função para listar os usuários dentro do useEffect
    }, []); // Passa um array vazio para garantir que o useEffect seja executado apenas uma vez ao montar o componente
 ...
 //Monta a coluna referente ao id
    ...
    const usuarioBodyTemplate = (rowData: Projeto.Usuario) => {
        return (
            <>
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable
                        ...
                        currentPageReportTemplate="Mostrando {primeiro} a {último} de {totalRecords} usuários"
                    ...
                    <Dialog visible={usuarioDialog} style={{ width: '450px' }} header="Detalhes de usuário" modal className="p-fluid" footer={usuarioDialogFooter} onHide={hideDialog}>
                        <div className="field">
                            ...
                            />
                            {submitted && !usuario.nome && <small className="p-invalid">Campo nome é obrigatório.</small>}
                        </div>
                        <div className="field">
                            ...
                            />
                            {submitted && !usuario.usuario && <small className="p-invalid">Campo usuario é obrigatório.</small>}
                        </div>
                        <div className="field">
                            ...
                            />
                            {submitted && !usuario.senha && <small className="p-invalid">Campo senha é obrigatório.</small>}
                        </div>
                        <div className="field">
                            ...
                            />
                            {submitted && !usuario.email && <small className="p-invalid">Campo email é obrigatório.</small>}
                        </div>
                    </Dialog>
